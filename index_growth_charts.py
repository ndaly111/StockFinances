#!/usr/bin/env python3
# index_growth_charts.py  –  FULL FILE  (v2025-07-13 f)
# -----------------------------------------------------------
...
#   guards, CI-safe matplotlib backend
# -----------------------------------------------------------
...
def _row(label, s):
    if s.empty:  # explicit dict fixes previous dict.fromkeys misuse
        return {
            "Metric": label,
            "Latest": "N/A",
            "Avg":    "N/A",
...
def _color_pct(val):   # ≤30 → green, ≥70 → red, else default
    try:
        v = float(val)
...
def _html(df, fname, pct_cols=None):
    ...
    path = os.path.join(OUT_DIR, fname)
    open(path, "w", encoding="utf-8").write(SUMMARY_CSS + sty.to_html())
    return path

# ───────── callable entrypoint / mini-main ─────────────────
