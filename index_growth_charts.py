#!/usr/bin/env python3
# index_growth_charts.py  –  FULL FILE  (v2025-07-13 k)
# -----------------------------------------------------------
# single, styled summary table  •  headless-safe charts
# -----------------------------------------------------------
...
# ───────── CSS (blue frame + grey header/grid) ─────────────
SUMMARY_CSS = """
<style>
...
def _pe_col(conn):
    cols, low = _columns(conn), {c.lower(): c for c in _columns(conn)}
...
def _row(label,s):
    ...
    return dict(Metric=label,Latest=r(lambda x:x.iloc[-1]),
...
def _write_summary(df,tk):
    html = (df.style
              ...
              .to_html())
    final = SUMMARY_CSS + html

    # ↓ add old growth-table filename so template picks up new style
    legacy_names = [f"{tk}_summary.html",
                    f"{tk}_implied_growth_summary.html",
                    f"{tk}_pe_ratio_summary.html",
                    f"{tk}_growth_tbl.html"]          # ← new line
...
def render_index_growth_charts(tk="SPY"):
    ...
    pe_png=_chart(pe_s,f"{tk} P/E Ratio","P/E",f"{tk}_pe_ratio.png")

    summary=_write_summary(
        pd.DataFrame([_row("Implied Growth (TTM)",ig_s),
                      _row("PE Ratio (TTM)",pe_s)]), tk)

    return {"implied_chart":ig_png,"pe_chart":pe_png,"summary_table":summary}
