...
USER_AGENT  = "SegmentScraper/1.1 (you@real-email.com)"  # SEC requires contact
PAUSE_SEC   = 0.25
YEARS_BACK  = 3
...
OPINC_TAGS  = ["SegmentReportingInformationOperatingIncomeLoss",
               "OperatingIncomeLoss"]
...
def fetch_concept(cik: str, concept: str) -> dict|None:
    url = f"https://data.sec.gov/api/xbrl/companyconcept/CIK{cik}/us-gaap/{concept}.json?dimension={AXIS}"
    r = requests.get(url, headers={"User-Agent": USER_AGENT}, timeout=30)
...
def fiscal_year(d: date) -> int:              # naive FY = calendar year of end-date
    return d.year

def keep_last_n_fy(df: pd.DataFrame, n: int) -> pd.DataFrame:
    if df.empty:
        return df.copy()
    fy_keep = sorted({fiscal_year(x) for x in df["end"]}, reverse=True)[:n]
    return df[df["end"].apply(lambda x: fiscal_year(x) in fy_keep)].copy()

# ── NEW: segment-catalog persistence ───────────────────────────────────────────
def load_catalog(ticker: str) -> dict[str, dict]:
...
def save_catalog(ticker: str, catalog: dict) -> None:
...
def update_catalog(ticker: str, df: pd.DataFrame) -> dict:
    ...
    for _, row in df.iterrows():
        ...
        if seg not in cat:
            ...
            if end < cat[seg]["first_seen"]:
                cat[seg]["first_seen"] = end
            if end > cat[seg]["last_seen"]:
                cat[seg]["last_seen"] = end
...
def plot_segments(df: pd.DataFrame, ticker: str) -> None:
    if df.empty:
        ...
        return
    # choose TTM if exists else latest FY
    label = "TTM" if "TTM" in df["fiscal_label"].values else str(max(int(x) for x in df["fiscal_label"] if x.isdigit()))
    d = df[df["fiscal_label"] == label]

    metrics = [m for m in ("revenue", "op_income") if m in d.columns]
    d_long  = d.melt(id_vars=["segment"], value_vars=metrics, var_name="metric", value_name="val")
...
def process_ticker(ticker: str, cik_map: dict[str, str]) -> None:
    ...
    full_df["fiscal_label"] = full_df["end"].apply(lambda d: "TTM" if ttm_df.size and d==latest else str(fiscal_year(d)))

    # NEW: ensure every historic segment appears
    catalog = update_catalog(ticker, full_df[["segment","end"]])
    for seg in catalog:
        if seg not in full_df["segment"].values:
            full_df.loc[len(full_df)] = [seg, latest, 0, 0, "TTM" if ttm_df.size else str(fiscal_year(latest))]
