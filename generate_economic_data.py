...
render an HTML table (economic_data.html) and time-series PNG charts.
...
# ───────────────────────── configuration ──────────────────────────
DB_FILE   = Path("Stock Data.db")          # same DB your site already uses
HTML_OUT  = Path("economic_data.html")     # consumed by html_generator2
CHART_DIR = Path("charts")                 # already exists in repo root

FRED_KEY  = os.getenv("FRED_API_KEY")      # add as GitHub Secret
...
def _render_html(meta_df: pd.DataFrame) -> None:
    ...
    for row in meta_df.to_dict(orient="records"):
        html.append(
            f"<tr><td>{row['name']}</td>"
            f"<td>{row['latest_value']}</td>"
            f"<td>{row['last_release']}</td>"
            f"<td>{row['next_release']}</td></tr>"
        )
...
def generate_economic_data() -> None:
    ...
    with sqlite3.connect(DB_FILE) as conn:
        ...
        for sid, meta in INDICATORS.items():
            ...
            conn.execute("""
                INSERT INTO economic_meta(indicator,name,units,source,last_release,next_release)
                VALUES(?,?,?,?,?,?)
                ON CONFLICT(indicator) DO UPDATE SET
                    last_release=excluded.last_release,
                    next_release=excluded.next_release""",
                (sid, meta["name"], meta["units"], meta["source"], last_date, next_rel)
            )
        ...
        _render_html(pd.DataFrame(summaries))
