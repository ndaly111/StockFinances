...
Outputs per ticker
  ...
  3) YoY expense-change HTML table (drops columns with no data)
"""
...
def pick_any(row: pd.Series, labels) -> float | None:
    for k in row.index:
...
def extract_expenses(row: pd.Series):
    return (
        pick_any(row, COST_OF_REVENUE),
        pick_any(row, RESEARCH_AND_DEVELOPMENT),
        pick_any(row, SELLING_AND_MARKETING),
        pick_any(row, GENERAL_AND_ADMIN),
        pick_any(row, SGA_COMBINED),
        pick_any(row, FACILITIES_DA),
        pick_any(row, PERSONNEL_COSTS),
        pick_any(row, INSURANCE_CLAIMS),
        pick_any(row, OTHER_OPERATING),
    )
    ...
    for idx, row in df.iterrows():
        ...
        cur.execute(
            ...
            (ticker, clean_value(pe), clean_value(row.get("Total Revenue")),
             clean_value(c), clean_value(r), clean_value(m), clean_value(a),
             clean_value(s), clean_value(f), clean_value(p),
             clean_value(i), clean_value(o)),
        )
    conn.commit(); cur.close()
    if own: conn.close()
...
def chart_abs(full, tkr):
    f = full.sort_values("year_int"); xl = f["year_label"].tolist()
...
def chart_pct(full, tkr):
    f = full.sort_values("year_int"); f = f[f["total_revenue"] != 0]
    ...
    ylim = max(110, (int(bot.max() / 10) + 2) * 10)
...
def generate_expense_reports(tkr, *, rebuild_schema=False, conn=None):
    ...
    # YoY table: drop fully-NaN columns, show blanks instead of "NaN"
    ...
    drop_cols = [c for c in yoy.columns[1:] if yoy[c].notna().sum() == 0]
    yoy = yoy.drop(columns=drop_cols)
    ...
    with open(os.path.join(OUTPUT_DIR, f"{tkr}_yoy_expense_change.html"), "w", encoding="utf-8") as f:
        f.write('<div class="scroll-table-wrapper">'
                + yoy.to_html(index=False, classes="expense-table", border=0, na_rep="")
                + '</div>')
    print(f"[{tkr}] charts & YoY table written")
