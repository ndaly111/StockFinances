...
def chart_pct(df: pd.DataFrame, ticker: str) -> None:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, f"{ticker}_expenses_pct_of_rev.png"), dpi=120, bbox_inches="tight")
    plt.close()
    print(f"[{ticker}] expense-% chart saved → {os.path.join(OUTPUT_DIR, f'{ticker}_expenses_pct_of_rev.png')}")
