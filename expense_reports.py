...
from expense_labels import (COST_OF_REVENUE, FACILITIES_DA, GENERAL_AND_ADMIN,
                            ...
                            PERSONNEL_COSTS, RESEARCH_AND_DEVELOPMENT,
                            SELLING_AND_MARKETING, SGA_COMBINED)
...
def _clean(v):
    """Convert timestamps to ISO or leave floats/ints as-is."""
    if pd.isna(v):
        return None
...
def _pick_any(row: pd.Series, labels: list[str]):
    """Return the first non-null column whose name contains any label substring."""
    for k in row.index:
...
def _extract_expenses(r: pd.Series):
    """Return a tuple of nine expense buckets (may include None)."""
    return (
...
# ───────────────────────── category meta ──────────────────────
# (label shown, dataframe column name, bar colour)
_CATEGORIES = [
...
def _store(tkr: str, mode: str = "annual", *, conn=None):
    ...
    for idx, row in df.iterrows():
        ...
        cur.execute(
            ...
            (tkr, _clean(pe), _clean(row.get("Total Revenue")), *[_clean(v) for v in vals]),
        )
    conn.commit()
    cur.close()
    if own:
...
def _cats(df: pd.DataFrame, combo_present: bool):
    ...
    if combo_present:  # hide S&M + G&A when a combined SG&A bucket exists
...
def _chart_pct(df: pd.DataFrame, tkr: str):
    ...
    # add percent columns
    for _, col, _ in cats:
        f[col + "_pct"] = pd.to_numeric(f[col], errors="coerce") / f["total_revenue"].astype(float) * 100
...
# columns used for tables (in order)
_TABLE_COLS = [
    "total_revenue",
    "cost_of_revenue",
...
def generate_expense_reports(tkr: str, *, rebuild_schema: bool = False, conn=None):
    ...
    # convert every numeric column once
    for col in _TABLE_COLS + ["total_revenue"]:
    ...
    _write_html(abs_fmt, os.path.join(OUTPUT_DIR, f"{tkr}_expense_absolute.html"))
    ...
    yoy.drop(columns=[c for c in yoy.columns[1:] if yoy[c].notna().sum() == 0], inplace=True)
    yoy = yoy[yoy.iloc[:, 1:].notna().any(axis=1)]

    rename_pct = {k: v.replace("($)", "Change (%)") for k, v in rename_abs.items()}
