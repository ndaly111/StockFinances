...
# Builds index.html + per-ticker pages + SPY/QQQ growth pages
# ───────────────────────────────────────────────────────────
...
def ensure_templates_exist():
    home_template_content = """..."""  # Omitted for brevity – keep your original content
    ticker_template_content = """..."""  # Omitted for brevity – keep your original content
...
# ───────────────────────────────────────────────────────────
# Generate SPY/QQQ growth standalone pages
# ───────────────────────────────────────────────────────────
def render_spy_qqq_growth_pages():
    ...
    for key, (template_name, summary_file, output_file) in env_paths.items():
        try:
            tpl_path = os.path.join('templates', template_name)
            html_template = open(tpl_path, 'r', encoding='utf-8').read()
            tpl = Template(html_template)

            summary_html = get_file_content_or_placeholder(os.path.join(chart_dir, summary_file), "No data available.")
            output = tpl.render(**{f"{key}_growth_summary": summary_html})
            ...
            print(f"Error rendering {key.upper()} growth page: {e}")
