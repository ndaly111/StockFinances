...
import os, re, sqlite3, pandas as pd, yfinance as yf, requests
...
def fetch_company_headlines(ticker: str, max_items: int = 6):
    ...
    if not items:
        cached = load_cached_headlines_from_page(ticker, max_items)
        if cached:
            print(f"[INFO] Using cached headlines for {ticker} from pages/{ticker}_page.html")
            return cached

    return items


def load_cached_headlines_from_page(ticker: str, max_items: int) -> list[dict]:
    """Attempt to load headlines from a previously generated page.

    This serves as an offline fallback when live RSS feeds are blocked. We
    intentionally keep the parsing regex simple because the cached HTML is
    generated by this project and uses predictable ``<li>``/``<a>`` markup.
    """

    path = os.path.join("pages", f"{ticker}_page.html")
    try:
        html = open(path, encoding="utf-8").read()
    except FileNotFoundError:
        return []
    except Exception as exc:
        print(f"[WARN] Unable to read cached headlines from {path}: {exc}")
        return []
