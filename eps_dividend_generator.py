...
def _ensure_schema(conn):
    ...
    if "ttm_dividend" not in [c[1].lower() for c in cur.fetchall()]:
...
def _build_chart(tic: str, conn: sqlite3.Connection) -> str:
    ...
    # dividends for those trailing years
    ...
    # ── assemble series  (trailing → TTM → forecast) ─────────────────
    labels, eps_hist, eps_fwd, divs = [], [], [], []
    for yr, eps in trailing:
        labels += [str(yr)]; eps_hist += [eps]; eps_fwd += [0]; divs += [div_map.get(yr, 0.0)]
    labels += ["TTM"]; eps_hist += [ttm_eps]; eps_fwd += [0]; divs += [ttm_div]
    for yr, fwd in forward:
        labels += [str(yr)]; eps_hist += [0]; eps_fwd += [fwd]; divs += [0.0]
...
def generate_eps_dividend(tickers, db_path=DB_PATH, chart_dir=CHART_DIR):
    ...
    for tic in tickers:
        ...
        if divs.empty or divs.sum() == 0:                     # no dividend
            os.makedirs(chart_dir, exist_ok=True)
            path = os.path.join(chart_dir, f"{tic}_eps_dividend_forecast.png")
            fig, ax = plt.subplots(figsize=(4,2), dpi=100)
            ax.text(0.5, 0.5, "no dividend", ha="center", va="center", fontsize=12)
            ax.axis("off"); fig.savefig(path, bbox_inches="tight", pad_inches=0); plt.close(fig)
            out[tic] = path; continue

        # store dividends & TTM in DB
        ...
        one_yr_total = divs[divs.index >= dt.datetime.utcnow()-dt.timedelta(days=365)].sum()
        _update_ttm_div(cur, tic, float(one_yr_total)); conn.commit()

        # build chart only from DB data
