...
def _build_chart(tic: str, conn: sqlite3.Connection, tkr: yf.Ticker) -> str:
    ...
    if missing_years:
        try:
            inc = tkr.get_income_stmt(freq="yearly", timeout=8)
            ...
            logging.warning(f"Data fetch failed for {tic}: {e}")
    ...
    for yr, val in reversed(trailing):
...
def generate_eps_dividend(tickers, db_path=DB_PATH, chart_dir=CHART_DIR):
    ...
    for tic in tickers:
        ...
        _update_ttm_div(cur, tic, float(divs[divs.index >= one_year_ago].sum()))
        conn.commit()
