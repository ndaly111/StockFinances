...
def save_yoy_growth_to_html(yoy_growth_table, charts_output_dir, ticker):
    ...
    styler = yoy_growth_table.style
    map_name = "map" if hasattr(styler, "map") else "applymap"
    styled_table = (getattr(styler, map_name)(style_negative)
                    .pipe(lambda s: getattr(s, map_name)(style_positive))
                    .set_table_styles({
