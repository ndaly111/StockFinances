...
def load_growth_data():
    ...
    Returns empty DF if DB or table is missing.
    """
...
def calculate_summary_stats(df: pd.DataFrame, col: str) -> dict:
    """
    Compute avg, median, std, current, and percentile (%) for col in df.
    Returns '-' placeholders if df empty or no values.
    """
...
def generate_summary_table(df: pd.DataFrame, ticker: str) -> str:
    """
    Create an HTML summary table for this ticker's implied-growth history,
    with one row per timeframe & growth_type.
    """
    df = df.copy()
...
def plot_growth_chart(df: pd.DataFrame, ticker: str) -> str:
    """
    Plot TTM & Forward implied-growth lines plus mean/median/±1σ bands,
    saving to a per-ticker PNG.
    """
    df = df.copy()
    df['date'] = pd.to_datetime(df['date_recorded'])
...
def generate_all_summaries():
    """
    Loops through each ticker in the history table and writes:
      - charts/{ticker}_implied_growth_summary.html
      - charts/{ticker}_implied_growth_plot.png
    """
    ensure_output_directory()
    df_all = load_growth_data()
    if 'ticker' not in df_all:
        return

    for t in df_all['ticker'].unique():
        df_t = df_all[df_all['ticker'] == t]
        generate_summary_table(df_t, t)
        plot_growth_chart(df_t, t)
